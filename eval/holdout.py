...
from eval.consumption import (STATE_SCORED, STATE_VALIDATED, STATE_VALIDATION_FAILED,
                              load_record, record_holdout_score)
...
def score_fixed_holdout(*, freeze_artifact: dict, record_path, contract: dict,
    ...
    checks it reproduces the recorded result hash — it never scores an unconsumed holdout
    ...
    if verify_only:
        ...
        result["reproduces_recorded_score"] = bool(
            recorded and recorded[-1]["result_sha256"] == result_sha256)
        return result

    record_holdout_score(record_path, freeze_artifact=freeze_artifact,
