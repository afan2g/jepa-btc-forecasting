...
def preflight_holdout_inputs(freeze_artifact: dict, *, contract: dict,
    ...
    for man, side in ((dev_manifest, "dev"), (holdout_manifest, "holdout")):
        ...
        # The frozen winner is always a CROSS-VENUE arm: both builds must satisfy the
        # same venue contract as the development arms (explicit roles, preregistered
        # Binance signal venues, exactly one Coinbase BTC-USD target) — the one-time
        # April score cannot be recorded under a manifest that omits or misdeclares
        # its sources.
        require_cross_venue_manifest(man, f"the {side} manifest")
