...
- **Dry-run (`--dry-run`):** metadata-only. Calls `lakeapi.list_data(...)` per `(feed, E, S)` (no parquet download), prints the resolved day set, per-feed presence/gaps, and the **estimated GB** from measured per-day sizes (Requirement 7). Writes the plan to the run report but performs **zero** parquet transfer. This is also the safe "manifest planning" entry point.
...
- **Region `eu-west-1`.** Build the session with `ingest/verify_lake.py::lake_session()` semantics: explicit `boto3.Session(aws_access_key_id=…, aws_secret_access_key=…, region_name=env.get("region","eu-west-1"))` from `.env` subscriber keys — **not** the default `~/.aws` chain (that is a different account → `AccessDenied`, docs §2.1). Pass the session into every `lakeapi` call (`boto3_session=sess`). For direct `pyarrow.fs.S3FileSystem`, pass the same keys + `region="eu-west-1"`.
- **`origin_time` first, documented `received_time` fallback.** Resolve the engine-time axis with the existing `recon.ingest.shared_engine_time_col(*dfs)` / `is_populated` (>99% populated). `origin_time` is measured 100%-populated for Binance `book_delta_v2` and `trades` (docs §5/§5b), so reconstruction runs on exchange time. If a feed/day is ever <99% populated, fall back to `received_time` (Binance Tokyo capture → 4.4 ms median lag, a tight proxy) and **record the fallback in the manifest** (`engine_time_col` field) so it is auditable, never silent. `origin_time` population for `funding`/`open_interest`/`liquidations` is **not yet measured** (Risk Q9) — the Phase-1 probe measures it, and the same `origin_time`→`received_time` fallback (with `engine_time_col` recorded) applies to those passthrough tables too, not just book/trades.
...
- **Top-K output contract.** Identical to the Coinbase path (`recon/native.py::_assemble`): columns `sample_ts, mid, microprice, bid_{i}_price, bid_{i}_size, ask_{i}_price, ask_{i}_size` for `i in 0..k-1` (default `k=10`). Emitted level prices are the **source floats**, never `tick/scale` round-trips. `meta` carries `seed_accepted, seed_ts, seed_reason, reseed_count, reseed_ts, reseed_blocked_invalid_snapshot, snapshot_reason_codes, crossed_rate, crossed_samples, missing_book_fraction, thin_depth_fraction, crossed_duration_s, policy` — written into the processed manifest per day.
...
- **Row counts / checksums / schema versions:** `rows` and `sha256` (of the final `data.parquet` bytes) go in the manifest and in the parquet key-value metadata; the schema version is a **per-store/per-output** constant — `RAW_SCHEMA_VERSION = "lake_binance/1"` for the normalized raw store and a `PROCESSED_SCHEMA_VERSION` map (`{"topk_l2": "topk_l2/1", "trades": "trades/1", …}`) for each processed output — bumped on any schema change. Reconciling manifest `rows`/`sha256` against re-read files is a cheap audit (AGENTS.md testing rules).
...
4. **Schema validation** (`test_lake_binance_schema.py`): a synthetic frame with the documented columns normalizes cleanly and stamps `schema_version`; a drifted frame (renamed/missing column, wrong dtype, unknown `side` value) raises a clear error listing seen columns (reuse the `recon.ingest._pick`/`_side_str` fail-loud pattern).
...
- **Phase 1 — metadata/schema probes + dry-run manifests.** `download_lake_binance.py --dry-run` (metadata only, no transfer) is free/unattended. The live probes are **approval-gated** (AGENTS.md — live vendor calls only when the user asks): `ingest/verify_lake.py` (metadata + 1-day `origin_time` check) and `ingest/verify_lake2.py` (**~2 GB** 2-yr coverage pull, docs §9), plus a bounded schema probe for `funding`/`open_interest`/`liquidations` and their `origin_time` population (Risk Q9), and tick-scale confirmation (Risk Q1). **No archive transfer.** Deliverable: schema fixtures, confirmed identifiers/ticks/`origin_time`, dry-run report.
...
- **Q7 — Quota/budget staging.** 18 mo Binance-only ≈ 476 GB > one 300 GB window; the archive pull must stage across ~2 windows with `used_data` re-checked each time. The `used_data` counter lags, so the plan's own GB estimate is the gate, not the vendor counter.
...
- [ ] **Step 1 — Failing test:** drive `process_unit(...)` with a fake reader yielding two small pyarrow batches → writes `data.parquet` atomically, appends an `ok` manifest record with `rows`/`sha256`/`schema_version`, and a second call skips (idempotent). Assert no network import is required to run the test.
- [ ] **Step 2 — Run red.**
- [ ] **Step 3 — Implement** the CLI: arg parsing (Requirement 8), `lake_session()` reuse, `--dry-run` via `lakeapi.list_data`, per-unit streaming normalize→ZSTD Parquet via `ParquetWriter` (batch loop like `download_coinapi.py:write_parquet`), retry/backoff, per-unit try/except → `status`, exit-code logic (0/2/3/4), report JSON. Keep `lakeapi`/`pyarrow` imports inside the vendor path so the pure helpers stay importable.
