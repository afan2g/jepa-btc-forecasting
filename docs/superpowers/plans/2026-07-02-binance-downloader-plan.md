...
| `tests/test_lake_binance_seed_source.py` | Create | Seed-source crossed-rate gate: `seed_source_crossed_frac > 5%` → `inconclusive`, no certified top-K (synthetic). |
...
- **Top-K output contract.** Identical to the Coinbase path (`recon/native.py::_assemble`) — **same column order**: `mid, microprice`, then `bid_{i}_price, bid_{i}_size, ask_{i}_price, ask_{i}_size` for `i in 0..k-1`, then `sample_ts` **last** (default `k=10`). Preserve this exact order so DataFrame-equality against the Python/native oracle holds (`_assemble` appends `sample_ts` last, `recon/native.py:266-274`) and the processed schema matches the Coinbase top-K contract. Emitted level prices are the **source floats**, never `tick/scale` round-trips. `meta` carries `seed_accepted, seed_ts, seed_reason, reseed_count, reseed_ts, reseed_blocked_invalid_snapshot, snapshot_reason_codes, seed_source_crossed_frac, crossed_rate, crossed_samples, missing_book_fraction, thin_depth_fraction, crossed_duration_s, policy` — written into the processed manifest per day (a day with `seed_source_crossed_frac > 5%` is stamped `inconclusive` and emits no certified top-K, Requirement 5).
...
- **Seed / reseed policy.** `book_delta_v2` starts mid-stream (no per-day snapshot block, docs §4.1) for Binance too, so seeding is required. Seed from the **locally-downloaded** Lake `book` 20-level snapshot product (`data/raw/lake/book/...`, the `SEED_PRODUCT` Stage 1 pulls alongside `book_delta_v2`, so Stage 2 stays quota-free) via `recon.reseed.snapshots_from_lake_book_df(...)` + `classify_snapshot(...)` (two-sided, finite/positive, ≥N levels, uncrossed, sane spread); reseed when the reconstructed book stays crossed ≥ `reseed_after_crossed_s`; reseed events apply at their own ts (no look-ahead); residual crossed samples stay visible in metrics.
- **Seed-source crossed-rate gate (mirrors the Coinbase quality-map contract).** Compute a day-level `seed_source_crossed_frac` — the fraction of thinned `book` seed candidates that are crossed — exactly as `run_coinbase_quality_map.py` does (docs §5a-QualityMap). A day whose seed source exceeds **5%** crossed is **not certifiable from Lake alone**: route it `inconclusive` (needs an alternate source) **even if an individual valid snapshot is accepted**, because on Coinbase an accepted seed off a >5%-crossed source stayed crossed for hours (docs §5a-QualityMap, lines 407-419 / 529-533). Record `seed_source_crossed_frac` in the processed manifest so a flaky source can never silently certify a top-K L2 output. Binance coverage/quality is higher than Coinbase, but this fraction is **unmeasured** for Binance (Risk Q3).
...
- **Q1 — Binance tick scales (RESOLVED 2026-07-11 — verified and registered; was: unverified, blocked native).** Native mode needs a verified `(exchange,symbol)→price_scale` where *every* price is an exact multiple of the tick (docs/native-recon.md). Expected — and since confirmed: perp tick $0.10 → `price_scale=10`, spot tick $0.01 → `price_scale=100`, measured against real data (min price increment across a sampled day) before being added to `recon/native.py::_TICK_SCALE`. *(RESOLVED 2026-07-11: the preregistered #64 tick-scale step measured both expected scales on Lake day `2026-04-01` — zero off-tick prices across `book_delta_v2`/`trades`/`book` for perp and spot (report_hash `d5025c58aa48…`, prereg commit `60a2b745`) — and issue #71 / PR #72 registered them in `_TICK_SCALE`, so `--engine auto` now selects native for exactly these two instruments when the extension is built. Explicit `python` remains the correctness oracle. Real-data native/Python conformance for both instruments was RECORDED on PR #72 (2026-07-12, from the #64 Stage-2 python/native runs on `2026-04-01`): byte-identical certified top-K parquet per instrument — equal sha256, identical quality/seed/classification blocks and raw-input identities; perp python 84,153 s (23.4 h) vs native 19.7 s at scale 10, spot python 13,731 s (3.8 h) vs native 7.9 s at scale 100.)*
...
**Files:** Create `scripts/run_binance_recon.py`; Modify `recon/native.py` (`_TICK_SCALE`, **only after Q1 verified**); Test `tests/test_binance_recon_conformance.py`, `tests/test_lake_binance_seed_source.py`.
...
- [ ] **Step 2 — Run red.**
- [ ] **Step 3 — Implement** the runner (read local normalized parquet per `(instrument,day)`; `resolve_engine`; emit top-K L2 + normalized tables; `--jobs` by `(instrument,day)`; processed manifest with `meta` including `seed_source_crossed_frac`). **Enforce the seed-source gate (Requirement 5):** a day with `seed_source_crossed_frac > 5%` is stamped `inconclusive` and emits no certified top-K frame. Add the **verified** Binance ticks to `_TICK_SCALE` (Q1) — if unverified, leave them out and let `auto` fall back (the test asserts the fallback, not the scale).
- [ ] **Step 4 — Run green** + `py_compile`.
- [ ] **Step 5 — Seed-source gate failing test** (`tests/test_lake_binance_seed_source.py`): a synthetic `book` seed frame with **>5% crossed** candidates → the day is classified `inconclusive`, `seed_source_crossed_frac` is recorded, and **no** certified top-K is emitted even when one individually-valid snapshot exists; a **<5%** source certifies normally (mirrors `run_coinbase_quality_map.py`; no vendor access). Run red → implement the gate in Step 3's runner → run green.
- [ ] **Step 6 — Commit.**

### Task 8: Docs cross-links

**Files:** `docs/data.md` §10 (**done in this PR:** the "Binance downloader — not yet built" open item now points to this plan). Remaining for the implementation branch: when the downloader lands, flip that item's status and optionally add a one-line pointer to this plan from `docs/native-recon.md` (note: the streaming-reader / day-parallelism follow-on text lives in `docs/superpowers/plans/2026-07-01-native-recon-engine.md` §Follow-on architecture, which native-recon.md links to — cite that; do not assume a follow-on note already exists in native-recon.md). Keep edits minimal; do not restate the plan.

- [ ] **Step 1** — (implementation branch) flip the §10 status once the downloader ships, `git diff --check`, commit.
...
.venv/bin/python -m pytest -q tests/test_lake_binance_paths.py tests/test_lake_binance_manifest.py \
    ...
    tests/test_lake_binance_seed_source.py \
...
.venv/bin/python ingest/download_lake_binance.py --instrument binance-spot --start 2026-04-01 --end 2026-04-01
.venv/bin/python scripts/run_binance_recon.py --instrument binance-perp --start 2026-04-01 --end 2026-04-01 --engine native
.venv/bin/python scripts/run_binance_recon.py --instrument binance-spot --start 2026-04-01 --end 2026-04-01 --engine native
```
