...
| `eval/freeze.py` | Hash-pinned selection artifact built strictly from development evidence: winner/config, resolved gate rules, frozen trade-validation thresholds, exact holdout scope (explicit day list — ranges/globs/months rejected), the contract's holdout window, source pins (contract, arm manifests, matched row/split hashes, **per-arm full-content matrix hashes** covering feature values), trial history. Only a passing G0-XV study authorizes a freeze. |
