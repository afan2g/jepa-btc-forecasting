...
| `eval/g0.py` | `run_g0cb_study` (development-only; no holdout parameter exists; holdout-bound manifests rejected before data) and `run_g0xv_development` (ONE unified study over matched arms; reserved-row + split hashes must be identical across arms; DSR uses the complete effective trial count including imported G0-CB history; PBO runs over the common cross-arm candidate-PnL matrix and fails closed when unavailable; the combined arm must beat the matched Coinbase-only control beyond a preregistered block-bootstrap noise band). |
| `eval/freeze.py` | Hash-pinned selection artifact built strictly from development evidence: winner/config, resolved gate rules, frozen trade-validation thresholds, exact holdout scope (explicit day list — ranges/globs/months rejected), source pins (contract, arm manifests, matched row/split hashes), trial history. Only a passing G0-XV study authorizes a freeze. |
| `eval/consumption.py` | One-time holdout transaction: `frozen → validated | validation_failed → scored`. Both #48's exact-scope trade validator and the scorer update it. Stale/regenerated artifacts, retries, partial-scope substitutions, and generic day selectors are rejected; a consumed or failed transaction can never be reused or replaced. |
| `eval/holdout.py` | `fit_frozen_config` (pre-holdout matched rows only, shared model definitions with `eval/baseline.py:fit_model`) + `score_fixed_holdout` (requires verified freeze + `validated` record; re-verifies contract/build/row-universe pins; scores once, marks consumed; `verify_only` reproduces an already-consumed score). |
| `scripts/run_g0.py` | Orchestrator CLI: `g0cb`, `g0xv-dev`, `freeze`, `holdout-open`, `holdout-validate`, `holdout-score`. G0-CB has no holdout arguments and fails on a holdout-bound manifest **before** the matrix file is opened; holdout scoring refuses before any holdout read unless the transaction is in the right state. |
