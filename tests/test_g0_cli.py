...
before any matrix read), G0-XV development rejects holdout-bound arms pre-load, and the
freeze -> open -> validate -> score flow enforces the one-time transaction end to end."""
import json
...
def test_g0xv_cli_rejects_holdout_bound_arm_before_data_loading(tmp_path, g0_world):
    store, f = _setup(tmp_path, g0_world)
    rc = rg.main(["g0xv-dev",
                  "--arm", f"coinbase_only={f['dev_coinbase_only_man']}:"
                           f"{f['dev_coinbase_only_mat']}",
                  "--arm", f"binance_only={f['dev_binance_only_man']}:"
                           f"{f['dev_binance_only_mat']}",
                  "--arm", f"combined={f['holdout_combined_man']}:"
                           f"{f['holdout_combined_mat']}",
                  "--contract", f["contract"], "--ledger", str(tmp_path / "l.json"),
                  "--out", str(tmp_path / "o.json")], read_matrix=store)
    assert rc == 2
    assert store.calls == []
...
def test_full_cli_flow_one_time_holdout(tmp_path, g0_world):
    ...
    for arm in ("coinbase_only", "binance_only", "combined"):
        arm_args += ["--arm", f"{arm}={f[f'dev_{arm}_man']}:{f[f'dev_{arm}_mat']}"]
    ...
    assert rg.main(["holdout-open", "--freeze", freeze_path, "--record", record],
                   read_matrix=store) == 0
    # scoring before validation must refuse BEFORE any holdout read
    pre_calls = list(store.calls)
    score_args = ["holdout-score", "--freeze", freeze_path, "--record", record,
                  "--contract", f["contract"],
    ...
    assert rg.main(["holdout-validate", "--freeze", freeze_path, "--record", record,
...
def test_cli_validation_failure_blocks_scoring_without_loading(tmp_path, g0_pipeline):
    ...
    rc = rg.main(["holdout-score", "--freeze", freeze_path, "--record", record,
